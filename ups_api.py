...
class UPSApiClient:
    ...
    def cancel_pickup(self, prn):
        ...
        # Official Documentation: DELETE /shipments/{version}/pickup/{CancelBy}
        # CancelBy '02' means cancel by PRN.
        url = f"{self.base_url.replace('/pickupcreation', '')}/shipments/v2409/pickup/02"
        headers = {
            'Authorization': f'Bearer {self.token}',
            'AccountNumber': self.account_number,
            'Prn': prn,
